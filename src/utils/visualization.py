...
def display_mask_channels_as_rgb(mask, channel_names, colors,title='Mask Channels as RGB'):
...
def plot_image_and_mask(image,mask,**kwargs):
    ...
    fig_masks = display_mask_channels_as_rgb(mask,colors=mask_region_colors,title=title,channel_names=region_names)
    #plot_overlay_masks_on_image(image, mask)
    vis.matplot(fig_imgs,opts={'resizable':True,'height':200,'width':400},win='img')
    vis.matplot(fig_masks,opts={'resizable':True,'height':200,'width':400},win='masks')
    plt.close(fig_imgs)
    plt.close(fig_masks)
    #time.sleep(1)
...
def plot_mask(mask,**kwargs):
    ...
    fig_masks = display_mask_channels_as_rgb(mask,colors=mask_region_colors,title=title,channel_names=region_names)
    vis.matplot(fig_masks,opts={'resizable':True,'height':200,'width':400},win=win)
    plt.close(fig_masks)

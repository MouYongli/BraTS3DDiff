...
class SynapseSubmissionCallBack(Callback):
    def __init__(self, 
        ...
        self.zip_file_name = f'submission_{datetime.now().strftime("%H_%M_%S")}' if not zip_file_name else zip_file_name
        self.submission_file_name = self.zip_file_name + '.zip'
        self.syn_submission_name = f"Submission_{datetime.now().strftime('%H_%M_%S')}" if not name else name
        self.syn_team = team
    ...
    def correct_file_paths_dic(self):
        ...
        directories = {'-'.join(f.split('-')[-2:]) for f in os.listdir(self.eval_images_path) if os.path.isdir(os.path.join(self.eval_images_path, f))}
        return directories
    ...
    def validate_file_names(self):
        ...
        assert os.path.exists(self.eval_images_path), "Eval Images Path Does Not Exists"
        assert os.path.exists(self.pred_labels_path), "Predicted Segmentation Masks Path Does Not Exists"
        ...
        for file_name in files:
            match = re.match(r'^.*?([a-zA-Z0-9_\-]+)-([a-zA-Z0-9_\-]+)\.nii\.gz$', file_name)
            if match:
...
if __name__ == "__main__":
    ...
    eval_images_path='/home/sanyal/Projects/BraTS3DDiff/data/BraTS-Data/BraTS2023-GLI/test'
    syn_eval_id='9615339'
    now = datetime.now()
    zip_file_name=f"BraTS23-SwinUNETR_baseline_0_{now:%Y-%m-%d}_{now:%H-%M-%S}"
    name=zip_file_name
    team=None
    syn_subm = SynapseSubmissionCallBack(auth_file=auth_file,

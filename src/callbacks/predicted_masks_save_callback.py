...
class PredictedMasksSaveCallBack(Callback):
    ...
    def mask_regions_to_labels(data_dir, mask_regions, labels, dim_order):
        ...
        if os.path.split(data_dir)[1] == "BraTS2023-GLI":
            p_wt, p_tc, p_et = (
                mask_regions[:, 0].unsqueeze(1),
                mask_regions[:, 1].unsqueeze(1),
                mask_regions[:, 2].unsqueeze(1),
            )
            ...
            p_et, p_netc, p_snfh, p_rc = (
                mask_regions[:, 2].unsqueeze(1),
                mask_regions[:, 3].unsqueeze(1),
                mask_regions[:, 4].unsqueeze(1),
                mask_regions[:, 5].unsqueeze(1),
            )
    ...
    def load_affine_and_header(data_dir, file_id, im_channels, sep, ext, split="test"):
        im_path = os.path.join(
            data_dir, split, file_id, f"{file_id}{sep}{im_channels[0]}{ext}"
        )

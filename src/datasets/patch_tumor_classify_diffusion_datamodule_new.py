...
from .transforms.transforms import ConvertToMultiChannelBasedOnBratsClassesd
...
def custom_collate(batch):
    ...
    img_shapes = []
    for item in batch:
        data, file_id, img_shape = item
        datas.append(data)
        file_ids.append(file_id)
        img_shapes.append(img_shape)

    datas = default_collate(datas)
    return datas, file_ids, img_shapes
...
class BraTSDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Any:
        ...
        if self.mode in ["train", "val"]:
            ...
            data, file_id, data_shape = self.read_data(self.image_path[index])
            data = self.transforms(data)
            return (data, file_id, data_shape)

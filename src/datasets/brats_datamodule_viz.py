...
class BraTSDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Any:
        ...
        if self.mode in ["train", "val"]:
            ...
            foreground = np.where(foreground > 0, 1, 0).astype(np.float32)
            data["foreground"] = foreground
            data["orig_foreground"] = foreground

            data["mask"] = self.separate_mask_labels_into_regions(data["mask"]).astype(
                np.uint8
            )
            data['orig_mask'] = data['mask']
            data = self.transforms(data)
            return data
    ...
    def __init__(
        ...
        self.train_transforms = Compose(
            [
                ...
                ToTensord(keys=["image", "mask", "foreground", "orig_image"]),
            ]
        )
        ...
        self.test_transforms = Compose(
            [   CropForegroundd(
                ...
                ToTensord(keys=["image", "mask", "foreground", "orig_image", "orig_mask", "orig_foreground"]),
            ]
        )

    ...
    def forward(self, patch_preds, patch_trues):
        ...
        if self.mode == "regress":
            ...
            for patch_res in self.patch_res:
                ...
                loss[f"patch_classify_loss_res={patch_res}"] = (
                    loss[f"patch_classify_dice_loss_res={patch_res}"]
                    + loss[f"patch_classify_ce_loss_res={patch_res}"]
                )
                loss["patch_classify_loss"] += loss[
                    f"patch_classify_loss_res={patch_res}"
                ]

...
class SwinUNETREnc128(nn.Module):
    ...
    def __init__(
        ...
        assert pred_mode in ("classify", "regress"), "pred_mode must be one of 'classify', 'regress'"
    ...
    def forward(self, x_in):
        ...
        assert x_3.shape[2:] == (128 // self.final_patch_sizes[0], ) * 3
        assert x_4.shape[2:] == (128 // self.final_patch_sizes[1], ) * 3

        if self.pred_mode == "regress":
            x_3_out = torch.clamp(x_3_out, 0, 1)
            x_4_out = torch.clamp(x_4_out, 0, 1)

        return {"embeddings": {self.final_patch_sizes[0]: x_3, self.final_patch_sizes[1]: x_4}, 
                    "labels": {self.final_patch_sizes[0]: x_3_out, self.final_patch_sizes[1]: x_4_out}}
...
if __name__ == "__main__":
    model = SwinUNETR(
        img_size=(128, 128, 64), in_channels=4, out_channels=3, depths=(2, 4, 2, 2)
    ).cuda()
    x = torch.randn(3, 4, 128, 128, 96).cuda()
    y = model(x)

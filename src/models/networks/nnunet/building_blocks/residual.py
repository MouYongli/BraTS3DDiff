...
from .regularization import DropPath, SqueezeExcite
...
class BasicBlockD(nn.Module):
    def __init__(
        ...
        has_stride = (isinstance(stride, int) and stride != 1) or any(
            [i != 1 for i in stride]
        )
            ...
            if has_stride:
                ops.append(
                    get_matching_pool_op(
                        conv_op=conv_op, adaptive=False, pool_type="avg"
                    )(stride, stride)
                )
    ...
    def compute_conv_feature_map_size(self, input_size):
        ...
        # conv1
        output_size_conv1 = np.prod(
            [self.output_channels, *size_after_stride], dtype=np.int64
        )
        # conv2
        output_size_conv2 = np.prod(
            [self.output_channels, *size_after_stride], dtype=np.int64
        )
...
class BottleneckD(nn.Module):
    ...
    def compute_conv_feature_map_size(self, input_size):
        ...
        # conv1
        output_size_conv1 = np.prod(
            [self.bottleneck_channels, *input_size], dtype=np.int64
        )
        # conv2
        output_size_conv2 = np.prod(
            [self.bottleneck_channels, *size_after_stride], dtype=np.int64
        )
        # conv3
        output_size_conv3 = np.prod(
            [self.output_channels, *size_after_stride], dtype=np.int64
        )
        # skip conv (if applicable)
        if (self.input_channels != self.output_channels) or any(
            ...
            output_size_skip = 0
        return (
            output_size_conv1 + output_size_conv2 + output_size_conv3 + output_size_skip
        )
...
class StackedResidualBlocks(nn.Module):
    def __init__(
        ...
        :param input_channels: only relevant for forst block in the sequence. This is the input
            number of features. After the first block, the number of features in the main path to
            which the residuals are added is output_channels
        :param output_channels: number of features in the main path to which the residuals are
            added (and also the number of features of the output)
        ...
        :param bottleneck_channels: if block is BottleneckD then we need to know the number of
            bottleneck features. Bottleneck will use first 1x1 conv to reduce input to bottleneck
            features, then run the nxn (see kernel_size) conv on that (bottleneck -> bottleneck).
            Finally the output will be projected back to output_channels (bottleneck ->
            output_channels) with the final 1x1 conv
        ...
        :param squeeze_excitation_reduction_ratio: ratio by how much squeeze and excitation should
            reduce channels respective to number of out channels of respective block
        """
        super().__init__()
        ...
        assert block in [
            BasicBlockD,
            BottleneckD,
        ], "block must be BasicBlockD or BottleneckD"

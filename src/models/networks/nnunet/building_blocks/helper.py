...
def get_matching_pool_op(
    ...
    """You MUST set EITHER conv_op OR dimension.
...
def maybe_convert_scalar_to_list(conv_op, scalar):
    """Useful for converting, for example, kernel_size=3 to [3, 3, 3] in case of nn.Conv3d :param
    conv_op:
    ...
    if not isinstance(scalar, (tuple, list, np.ndarray)):
        if conv_op == nn.Conv2d:
            ...
            raise RuntimeError(f"Invalid conv op: {conv_op.__repr__}")
    else:
        return scalar
...
def get_default_network_config(
    ...
    config = {'conv_op': torch.nn.modules.conv.Conv2d,           'dropout_op':
    torch.nn.modules.dropout.Dropout2d,           'norm_op':
    torch.nn.modules.batchnorm.BatchNorm2d,           'norm_op_kwargs': {'eps': 1e-05, 'affine':
    True},           'nonlin': torch.nn.modules.activation.ReLU,           'nonlin_kwargs':
    {'inplace': True}}

    There is no need to use get_default_network_config. You can create your own. Network configs
    are a convenient way of setting dimensionality, normalization and nonlinearity.

    :param dimension: integer denoting the dimension of the data. 1, 2 and 3 are accepted
    :param nonlin: string (ReLU or LeakyReLU)
    :param norm_type: string (bn=batch norm, in=instance norm) torch.nn.Module
    :return: dict
    """
    config = {}

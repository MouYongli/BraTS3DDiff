...
class PatchTumorDiffusionLitModule(LightningModule):
    ...
    def __init__(
        ...
        self.patch_criterion = PatchTumorLoss(
            mode="classify", patch_res=self.hparams.extra_kwargs.patch_sizes, avg=True
        )
    ...
    def model_step(self, batch) -> th.Tensor:
        ...
        patches_classify_loss_dict = self.patch_criterion(
            patches_pred_labels, patch_tumor_labels
        )
        ...
        for i, patch_size in enumerate(patch_sizes):
            ...
            patch_embeddings = (
                patch_embeddings.permute(0, 2, 3, 4, 1)
                .contiguous()
                .view(-1, 1, patch_emb_size, patch_emb_size, patch_emb_size)
            )
            ...
            mask_patch = (
                mask_patch.permute(0, 2, 4, 6, 1, 3, 5, 7)
                .contiguous()
            ...
            foreground_patch = foreground.view(
                B, 1, W_, patch_size, H_, patch_size, D_, patch_size
            )
            foreground_patch = (
                foreground_patch.permute(0, 2, 4, 6, 1, 3, 5, 7)
                .contiguous()
                .view(-1, 1, patch_size, patch_size, patch_size)
            )
            ...
            patch_seg_losses = self.criterion(
                pred_mask_patch * foreground_patch, mask_patch
            )
            ...
            patch_pred_labels = (
                patch_pred_labels.view(B, 1, W_, 1, H_, 1, D_, 1)
                .expand(B, C, W_, patch_size, H_, patch_size, D_, patch_size)
                .contiguous()
                .view(B, C, W, H, D)
            )
            ...
            loss_dict["window_seg_loss"] += loss_dict[
                f"window_seg_loss_res={patch_size}"
            ]
        ...
        loss_dict["loss"] = (
            loss_dict["patch_classify_loss"]
            + loss_dict["patch_seg_loss"]
            + loss_dict["window_seg_loss"]
            + loss_dict["window_seg_loss_res=mean"]
        )
        return loss_dict
    ...
    def validation_step(self, batch):
        ...
        for patch_size in patch_sizes:
            pred_scores, dice = compute_subregions_pred_metrics(
                ...
                1,
                subregions_names,
                prefix_key="patch_classify",
                suffix_key={"res": patch_size},
            )
        ...
        mean_pred_scores, mean_pred_dice = compute_subregions_pred_metrics(
            mean_pred_mask * foreground,
            mask,
            C,
            subregions_names,
            prefix_key="seg",
        ...
        self.log(
            f"val/dice", mean_seg_dice, on_step=False, on_epoch=True, prog_bar=True
        )

        classify_and_seg_dice = (
            val_metrics["patch_classify_dice"]
            + val_metrics["seg_dice"]
            + mean_pred_dice
        ) / 3
        self.log(
            f"val/class+seg_dice",
            classify_and_seg_dice,
            on_step=False,
            on_epoch=True,
            prog_bar=True,

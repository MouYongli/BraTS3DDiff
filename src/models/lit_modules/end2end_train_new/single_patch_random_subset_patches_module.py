...
class BraTSPatchTumorDiffusionLitModule(LightningModule):
    ...
    def __init__(
        ...
        self.classify_patch_sizes = self.hparams.extra_kwargs.classify_patch_sizes
        ...
        self.patch_classify_criterion = MultiResPatchClassifyLoss(
            mode="classify", patch_res=self.classify_patch_sizes, scale_loss=None
        )
        ...
        self.patch_classify_metric = MultiResPatchClassifyMetrics(patch_sizes=self.classify_patch_sizes,
    ...
    def _init_linear_schedule_epss(self):
        self.epss = np.concatenate(
            (
                np.linspace(self.start_eps, self.end_eps, self.num_epochs_use),     
            self.end_eps*np.ones(self.trainer.max_epochs-self.num_epochs_use)
        ))
    ...
    def on_train_start(self):
        ...
        self.eps_data = self.hparams.extra_kwargs.eps_data #{'eps_mode': , 'eps'}
        if self.eps_data['eps_mode'] == 'fixed':
            self.eps = self.eps_data['eps']

        elif self.eps_data['eps_mode'] == 'schedule':
            self.eps = None
            self.start_eps = float(self.eps_data['start_eps'])
            self.end_eps = float(self.eps_data['end_eps'])
            self.num_epochs_use = self.eps_data['num_epochs_use']
            self._init_linear_schedule_epss()


    def on_train_epoch_start(self):
        if self.eps_data['eps_mode'] == 'schedule':
            self.eps = self.epss[self.trainer.current_epoch]
    ...
    def model_step(self, batch) -> torch.Tensor:
        ...
        patch_embeddings, patch_pred_labels = self.forward(
            image=image, patch_sizes=self.classify_patch_sizes, pred_type="patchify"
        )
        ...
        self.log('eps', self.eps, on_epoch=True, on_step=True, prog_bar=True)
        ...
        masked_seg_loss, _ = self.segment_criterion(masked_pred_mask_patch, mask_patches, prefix='masked_seg')
    ...
    def predict_seg_mask(self, image, fwd_patch_sizes=None, ret_patch_labels=False):
        """
        ...
        if not fwd_patch_sizes:
            fwd_patch_sizes = self.classify_patch_sizes
        # get patch embeddings and pred patch labels from SwinT encoder
        patch_embeddings, patch_pred_labels = self.forward(
            image=image, patch_sizes=fwd_patch_sizes, pred_type="patchify"
        )
    ...
    def test_step(self, batch):
        ...
        pred_seg_masks = self.inferer(inputs=image, network=self.predict_seg_mask, fwd_patch_sizes=[self.seg_patch_size])

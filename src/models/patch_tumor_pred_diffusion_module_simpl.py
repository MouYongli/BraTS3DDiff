...
from src.models.networks.unet.patch_denoise_unet import PatchDenoiseUNet
...
class PatchTumorDiffusionLitModule(LightningModule):
    ...
    def forward(self, **kwargs):
        ...
        def _ddim_sample(mask_patch_shape, patch_size, patch_embeddings):
            sample_out = self.sample_diffusion.ddim_sample_loop(self.net, mask_patch_shape, \
                                                    model_kwargs={"image": patch_embeddings, "embeddings": None, "patch_size": patch_size})
            sample_out = sample_out["sample"]
            return sample_out

        def _ddim_sample_uncer_aware(mask_patch_shape, patch_size, patch_embeddings):
            #uncertainty fusion based from diffunet
            uncer_step = self.hparams.extra_kwargs.uncer_step
            sample_outputs = []
            for i in range(uncer_step):
                sample_outputs.append(self.sample_diffusion.ddim_sample_loop(self.net, mask_patch_shape, \
                                                                model_kwargs={"image": patch_embeddings, "embeddings": None, "patch_size": patch_size},viz_kwargs=None))
            ...
            return sample_return.to(patch_embeddings)
        ...
        if pred_type == "q_sample":
            ...
            return _ddim_sample(kwargs.get('mask_patch_shape'), kwargs.get('patch_size'), kwargs.get('patch_embeddings'))

        elif pred_type == "ddim_sample_uncer_aware":
            return _ddim_sample_uncer_aware(kwargs.get('mask_patch_shape'), kwargs.get('patch_size'), kwargs.get('patch_embeddings'))
    ...
    def model_step(self, batch) -> th.Tensor:
        ...
        loss_dict = {'patch_seg_loss':0.0,'window_seg_loss':0.0}
        ...
        for i, patch_size in enumerate(patch_sizes):
            ...
            x_start = (mask_patch) * 2 - 1

            t, weights = self.schedule_sampler.sample(x_start)
            x_t, noise = self.forward(x_start=x_start, t=t, pred_type="q_sample")
            denoise_out = self.forward(x_t=x_t, t=t, patch_embeddings=patch_embeddings, patch_size=patch_size, pred_type="denoise_out")

            #patch denoising loss
            ...
            pred_mask_patch = self.forward(x_t=x_t, t=t, denoise_out=denoise_out, pred_type="pred_xstart")

            ##Compute patch seg losses b/w predicted mask patches and true mask patch
            patch_seg_losses = self.criterion(pred_mask_patch * foreground_patch, mask_patch)
            ...
            loss_dict[f"patch_seg_dice_loss_res={patch_size}"] = patch_loss_dice
            loss_dict[f"patch_seg_ce_loss_res={patch_size}"] = patch_loss_bce
            loss_dict[f"patch_deno_loss_res={patch_size}"] = deno_loss
            loss_dict[f"patch_seg_loss_res={patch_size}"] = patch_loss
            loss_dict['patch_seg_loss'] += loss_dict[f"patch_seg_loss_res={patch_size}"]


            #Compute Window level seg loss

            # Reshape the predicted mask patches to get the window level mask
            ...
            window_seg_losses = self.criterion(pred_mask*foreground, mask)
            ...
            loss_dict[f"window_seg_dice_loss_res={patch_size}"] = wloss_dice
            loss_dict[f"window_seg_ce_loss_res={patch_size}"] = wloss_bce
            loss_dict[f"window_seg_loss_res={patch_size}"] = wloss
            loss_dict['window_seg_loss'] += loss_dict[f"window_seg_loss_res={patch_size}"]

        #scale loss by no. of patch resolutions
        loss_dict['patch_seg_loss'] /= len(patch_sizes)
        loss_dict['window_seg_loss'] /= len(patch_sizes)

        #Mean of mask window predictions using all the patch_sizes
        mean_pred_mask = torch.mean(torch.stack(pred_masks), dim=0)
        window_seg_losses = self.criterion(mean_pred_mask*foreground, mask)
        wloss_dice = window_seg_losses['dice_loss']
        wloss_bce = window_seg_losses['bce_loss']
        wloss = wloss_dice + wloss_bce
        #self.manual_backward(wloss)

        loss_dict[f"window_seg_dice_loss_res=mean"] = wloss_dice
        loss_dict[f"window_seg_ce_loss_res=mean"] = wloss_bce
        loss_dict[f"window_seg_loss_res=mean"] = wloss

        loss_dict['loss'] = loss_dict['patch_classify_loss'] + loss_dict['patch_seg_loss'] + loss_dict['window_seg_loss'] + loss_dict["window_seg_loss_res=mean"]
        return loss_dict
        ...
        for i, patch_size in enumerate(patch_sizes):
            ...
            if num_tumor_patches > 0:
                ...
                pred_tumor_mask_patch = self.forward(pred_type=self.hparams.extra_kwargs.sampling_type, mask_patch_shape=mask_patch_shape, \
                                                        patch_size=patch_size, patch_embeddings=tumor_patch_embeddings)
    ...
    def validation_step(self, batch):
        ...
        for i, patch_size in enumerate(patch_sizes):
            ...
            seg_metrics[f"seg_dice_loss_res={patch_size}"] = loss_dice
            seg_metrics[f"seg_ce_loss_res={patch_size}"] = loss_bce
            seg_metrics[f"seg_loss_res={patch_size}"] = loss
        ...
        seg_metrics['seg_loss'] /= len(patch_sizes)
        seg_metrics['seg_dice'] /= len(patch_sizes)
        ...
        mean_pred_loss_dice = mean_pred_seg_losses['dice_loss']
        mean_pred_loss_bce = mean_pred_seg_losses['bce_loss']
        mean_pred_loss = mean_pred_loss_dice + mean_pred_loss_bce

        seg_metrics[f"seg_dice_loss_res={patch_size}"] = mean_pred_loss_dice
        seg_metrics[f"seg_ce_loss_res={patch_size}"] = mean_pred_loss_bce
        seg_metrics[f"seg_loss_res={patch_size}"] = mean_pred_loss
        ...
        mean_seg_dice = 0.5*(val_metrics['seg_dice'] + mean_pred_dice)
        self.log(f"val/dice", mean_seg_dice, on_step=False, on_epoch=True, prog_bar=True)

        classify_and_seg_dice = (val_metrics['patch_classify_dice'] + val_metrics['seg_dice'] + mean_pred_dice)/3
